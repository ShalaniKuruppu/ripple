...
async def segment(
    ...
    try:
        ...
        if output_json["status"] == "success":
            # Return the nested JSON data directly as text/plain
            # Frontend will parse it to extract mask_image
            return Response(content=output_json["data"], media_type="application/json")
        else:
